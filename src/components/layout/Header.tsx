 ...
 */
const FILTERS_SEEN_KEY = "budget-demo.header.filtersSeen";
...
export function Header() {
  const { dataOrigin, config, transactions, status } = useDataStore();
  const cat1Options = status === "success" ? CAT1_OPTIONS.filter(c => c.value === "all" || transactions.some(t => t.cat1 === c.value)) : CAT1_OPTIONS;
      ...
      <div className="flex items-center justify-between gap-2 px-3 py-2 xs:px-6 xs:py-3">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <span className="text-base xs:text-xl font-title font-bold text-text truncate">
            {isSmall || dataOrigin !== "static" ? "Mon Budget" : "Mon Budget — Démo"}
          </span>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <a href="#/parametres" aria-label="Paramètres" className="inline-flex items-center justify-center gap-1.5 min-h-tap min-w-tap px-2 text-text-sec hover:text-text"><Settings size={18} /><span className="hidden sm:inline text-xs">Paramètres</span></a>
          <UploadButton />
        </div>
      </div>
