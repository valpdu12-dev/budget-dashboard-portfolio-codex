...
export function SubNav() {
  ...
  const { hasSalary, hasInflation, hasLoan, hasSavings } = useAvailableFeatures();

  if (!pills) return null;

  // Les fonctions facultatives ne sont proposées que lorsque la source
  // courante fournit effectivement les données nécessaires.
  const visiblePills = pills.filter((pill) => {
    if (pill.path === "/revenus/salaire") return hasSalary;
    if (pill.path === "/revenus/inflation") return hasInflation;
    if (pill.path === "/patrimoine") return hasSavings;
    if (pill.path === "/patrimoine/pret") return hasLoan;
    return true;
  });
