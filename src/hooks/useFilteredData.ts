...
export function useFilteredData() {
  ...
  const allMonths = useMemo(() => {
    const months = new Set([
      ...
      ...coverage.completeMonths,
    ]);
    return [...months].sort();
  }, [transactions, coverage.completeMonths]);
  ...
  const baseTx = useMemo(() => {
    return rawPeriodTx.filter((t) => {
      if (!showTransfers && isTransfer(t)) return false;
  ...
  const filteredTx = useMemo(() => {
    ...
    if (selOrg) tx = tx.filter((t) => (t.accountId ?? toOrganisme(t.compte)) === selOrg);
    return tx;
