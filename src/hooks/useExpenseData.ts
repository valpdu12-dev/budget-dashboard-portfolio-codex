...
import { isTransfer, matchesType, typeKey, cents } from "@/utils/businessRules";
...
export function useExpenseData() {
  ...
  const allTransactions = useDashboardTransactions();
  const organismes = useMemo(() => config?.accounts?.map(a => ({ id: a.id, label: a.label })) ?? ORGANISMES.map(label => ({ id: label, label })), [config]);
  const orgKeys = useMemo(() => organismes.map(o => o.id), [organismes]);
  ...
  const expMonthlyLines = useMemo<MonthlyLineRow[]>(() => {
    ...
    if (selType) tx = tx.filter((t) => matchesType(t, selType));

    const rows = allMonthsInRange.map((mk) => {
      const mTx = tx.filter((t) => t.monthKey === mk && t.dc === "Débit" && !isTransfer(t));
      const row: MonthlyLineRow = { name: mkLabel(mk), mk, Total: 0 };
      orgKeys.forEach((org) => {
        row[org] = cents(
          mTx
            .filter((t) => (t.accountId ?? toOrganisme(t.compte)) === org)
            .reduce((s, t) => s + t.montant, 0)
        );
      });
      row.Total = cents(orgKeys.reduce((s, o) => s + ((row[o] as number) || 0), 0));
      return row;
    });
    // Ajouter prev_* pour comparaison N-1 dans les tooltips
    for (let i = 1; i < rows.length; i++) {
      const prev = rows[i - 1];
      orgKeys.forEach((org) => { rows[i][`prev_${org}`] = prev[org]; });
      rows[i]["prev_Total"] = prev.Total;
    }
    return rows;
  }, [allMonthsInRange, baseTx, selCat2, selType, orgKeys]);

  // ── 2. Répartition par Cat2 (Donut) ───────────────────────────────
  const expByCat2 = useMemo<Cat2Slice[]>(() => {
    const map: Record<string, number> = Object.create(null);
    filteredTx
      .filter((t) => t.dc === "Débit" && !isTransfer(t) && t.cat2 && t.cat2 !== "x")
      .forEach((t) => {
  ...
  const expByType = useMemo<TypeSlice[]>(() => {
    const map: Record<string, number> = Object.create(null);
    filteredTx
      .filter((t) => t.dc === "Débit" && !isTransfer(t) && t.type)
      .forEach((t) => {
        const id = typeKey(t);
        map[id] = (map[id] || 0) + t.montant;
      });
    return Object.entries(map)
      .map(([id, value]) => {
        const name = filteredTx.find(t => typeKey(t) === id)!.type;
        return { id, name: name.length > 40 ? name.slice(0, 37) + "…" : name, fullName: name, value: cents(value) };
      })
      .sort((a, b) => b.value - a.value);
  }, [filteredTx]);
  ...
  const detailRows = useMemo<DetailRow[]>(() => {
    return filteredTx
      .filter((t) => t.dc === "Débit" && !isTransfer(t))
      .map((t) => ({
        date: t.date,
        label: t.label,
        type: t.type,
  ...
  const topMerchants = useMemo<TopItem[]>(() => {
    const map: Record<string, number> = Object.create(null);
    filteredTx
      .filter((t) => t.dc === "Débit" && !isTransfer(t) && t.cat3 && t.cat3 !== "x")
      .forEach((t) => {
  ...
  const compNvsN1 = useMemo<CompNvsN1Row[]>(() => {
    ...
    const estDepense = (t: Transaction) =>
      t.dc === "Débit" && !isTransfer(t) && !!t.cat2 && t.cat2 !== "x";
    ...
    const yPrev = allTransactions.filter(
      (t) =>
        ...
        (!selType || matchesType(t, selType)) &&
        (!selOrg || (t.accountId ?? toOrganisme(t.compte)) === selOrg)
    );

    const mCur: Record<string, number> = Object.create(null);
    const mPrev: Record<string, number> = Object.create(null);
    ...
    return Object.keys({ ...mCur, ...mPrev })
      .map((k) => ({
        name: k,
        [curYear]: cents(mCur[k] || 0),
        [prevYear]: cents(mPrev[k] || 0),
      }))
