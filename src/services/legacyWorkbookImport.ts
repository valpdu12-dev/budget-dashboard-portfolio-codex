...
type LegacyTransaction = Omit<Transaction, "accountId" | "typeId" | "role">;
...
function parseLegacyTransactions(workbook: XLSX.WorkBook, issues: ImportIssue[]): { transactions: LegacyTransaction[]; sheets: string[] } {
  ...
  for (const name of sheets) {
    ...
    rows.slice(1).forEach((row, index) => {
      ...
      let montant: number | null = typeof row[5] === "number" && Number.isFinite(row[5]) ? round(Math.abs(row[5])) : null;
      if (montant == null && typeof row[4] === "number" && Number.isFinite(row[4])) {
        montant = round(Math.abs(row[4]) / (isHalfAccount(compte) ? 2 : 1));
      }
      if (montant == null) {
...
function normalizeLegacyTransactions(source: LegacyTransaction[], mainAccount: string): { transactions: LegacyTransaction[]; savingsAccounts: Set<string> } {
  ...
  let transferIndex = 0;
  const pair = (first: LegacyTransaction, secondAccount: string, secondDc: "Débit" | "Crédit") => {
    const transferId = `legacy-transfer-${++transferIndex}`;
    normalized.push({ ...first, transferId });
    normalized.push({ ...first, label: `Contrepartie — ${first.label}`, compte: secondAccount, dc: secondDc, cat1: "", cat2: "", cat3: "", cat4: "", ville: "", transferId });
  };
  ...
  for (const transaction of source) {
    if (isTransferCreditType(transaction.type)) {
      ...
      pair({ ...transaction, compte: mainAccount, dc: "Débit" }, savingsAccountFor(transaction.type), "Crédit");
    } else {
...
function workbookEndingBalances(workbook: XLSX.WorkBook): { balances: Record<string, number>; row: number } {
  const sheet = workbook.Sheets.Visualisation;
  if (!sheet) return { balances: {}, row: 1 };
  ...
  if (headerIndex < 0 || balanceIndex < 0) return { balances: {}, row: 1 };
  const balances: Record<string, number> = {};
  rows[headerIndex].forEach((value, column) => {
    const account = clean(value);
    const balance = rows[balanceIndex]?.[column];
    if (column > 0 && account && typeof balance === "number" && Number.isFinite(balance)) balances[account] = round(balance);
  });
  return { balances, row: balanceIndex + 1 };
}
...
export function parseLegacyWorkbook(workbook: XLSX.WorkBook): { dataset: ImportDataset; validation: ValidationReport } {
  ...
  const mainAccount = inferMainAccount(sourceTransactions, Object.keys(ending.balances));
  ...
  const accountLabels = [mainAccount, ...detectedAccounts.filter(account => account !== mainAccount).sort()];
  ...
  const accounts = accountLabels.map((label, index) => ({ id: `legacy-account-${String(index + 1).padStart(3, "0")}`, label, initialBalance: init[label], kind: accountKinds[label], share: 100 }));
  ...
  const config: Config = {
    version: 2, accounts, types, perspective: "bank", init, accountKinds,
  ...
  return {
    ...
    validation: {
      ...
      comptes: accountLabels, totalDebits, totalCredits, net: round(totalCredits - totalDebits),
