...
import { isTransfer, typeKey, matchesType, typeLabel } from "@/utils/businessRules";
...
export default function Recettes() {
  ...
  const recettes = useMemo(() => baseTx.filter((t) => t.dc === "Crédit" && !isTransfer(t)), [baseTx]);
  ...
  const { lineData, types } = useMemo(() => {
    const map: Record<string, Record<string, number | string>> = Object.create(null);
    const typesSet = new Set<string>();
    recettes.forEach((t) => {
      if (!map[t.monthKey]) map[t.monthKey] = { monthKey: t.monthKey };
      const tp = typeKey(t) || "Autre";
      typesSet.add(tp);
  ...
  const donutData = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([id, value]) => ({ id, name: recettes.find(t => typeKey(t) === id)?.type ?? id, value }))
  ...
  const tableData = useMemo(() => {
    ...
    if (selRecMonth) src = src.filter((t) => t.monthKey === selRecMonth);
    if (selRecType) src = src.filter((t) => matchesType(t, selRecType));
    return src;
    ...
    <div className="flex flex-col gap-6">
      ...
      {hasFilters && (
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {selRecType && (
            <Chip label={typeLabel(useDataStore.getState().config, selRecType)} active onClear={() => setSelRecType(null)} />
          )}
          <button
            ...
            >
              ...
              {types.map((tp, i) => (
                <Line
                  key={tp}
                  type="monotone"
                  dataKey={tp} name={typeLabel(useDataStore.getState().config, tp)}
              ...
              <ResponsiveContainer width={isSmall ? "100%" : 260} height={chartHeight(260)}>
                <PieChart tabIndex={-1}>
                  <Pie rootTabIndex={-1}
                    ...
                    labelLine={false}
                    onClick={(_, idx) => setSelRecType(donutData[idx]?.id || null)}
