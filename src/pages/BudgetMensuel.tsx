...
// Saisie inline des budgets cibles → sauvegarde locale dans le navigateur.
...
const STATUS_STYLES: Record<BudgetCat2Row["status"], { bg: string; text: string; label: string }> = {
  ...
  "insufficient-data": { bg: "bg-amber/10", text: "text-amber", label: "Données insuffisantes" },
};
...
function BudgetInput({
  ...
  cat2: string;
  value: number | null;
  onSave: (cat2: string, target: number) => Promise<void>;
}) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      await onSave(cat2, Math.round(n));
    } catch (e) {
...
export default function BudgetMensuel() {
  ...
  const {
    rows, kpis, topRows, isLoading, coverage, comparableMonths, updateBudget,
  } = useBudgetData();
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {rows.length > 0 && comparableMonths.length === 0 && (
        <div
          role="status"
          className="flex items-start gap-2 px-3.5 py-2.5 bg-amber/[0.08] border border-amber/25 rounded-lg text-[13px] text-amber"
        >
          <span>
            Aucun mois complet n'est confirmé pour la période sélectionnée. Les moyennes,
            écarts et indicateurs de conformité restent indisponibles au lieu d'être
            calculés comme des zéros.
            {coverage.basis === "inferred" && (
              <span className="text-text-sec">
                {" "}La couverture est actuellement déduite prudemment des dates des mouvements.
              </span>
            )}
          </span>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 stagger-grid">
        ...
        <KPICard
          ...
          color={kpis.balance === null ? undefined
            : kpis.balance >= 0 ? "#22c55e" : "#ef4444"}
        />
        <KPICard
