...
import { matchesType, typeLabel } from "@/utils/businessRules";
import { EPARGNE_COLORS, DONUT_COLORS } from "@/config/colors";
...
const getTypeColor = (name: string, idx: number) =>
  EPARGNE_COLORS[name] || DONUT_COLORS[idx % DONUT_COLORS.length];
  ...
  },
  {
    ...
    render: (v: unknown, row: Record<string, unknown>) => (
      <span className={row.isSortie ? "text-red" : "text-text-sec"}>
        {row.isSortie ? "Retrait épargne" : String(v)}
      </span>
    ),
  },
  {
...
export default function Epargne() {
  ...
  const filteredTableData = useMemo(() => {
    ...
    if (selEpType) data = data.filter((t) => !t.isSortie && matchesType(t, selEpType));
    return data;
    ...
    <div className="flex flex-col gap-6">
      ...
      {hasFilters && (
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {selEpType && (
            <Chip label={typeLabel(useDataStore.getState().config, selEpType)} active onClear={() => setSelEpType(null)} />
          )}
            ...
            <ResponsiveContainer width={isSmall ? "100%" : 260} height={chartHeight(260)}>
              <PieChart tabIndex={-1}>
                <Pie rootTabIndex={-1}
                  ...
                  onClick={(_: unknown, idx: number) => setSelEpType(donutData[idx]?.id || null)}
                  className="cursor-pointer"
                >
                  {donutData.map((d, i) => (
                    <Cell key={i} fill={getTypeColor(d.name, i)} />
                  ))}
                </Pie>
              ...
              {donutData.map((d, i) => (
                <li key={d.name} className="flex items-center gap-1.5 text-xs text-text-sec">
                  <span
                    ...
                    style={{ backgroundColor: getTypeColor(d.name, i) }}
                    aria-hidden="true"
                  />
