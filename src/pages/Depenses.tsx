...
import { accountLabel, typeLabel, isTransfer } from "@/utils/businessRules";
import { ORG_COLORS, CAT2_COLORS, DONUT_COLORS, TYPE_COLORS } from "@/config/colors";
...
export default function Depenses() {
  ...
  const { filteredTx, allMonthsInRange, currentMonth, prevMonth } = useFilteredData();
  ...
  // ── KPIs ──────────────────────────────────────────────────────────────
  const kpis = useMemo(() => {
    const debits = filteredTx.filter((t) => t.dc === "Débit" && !isTransfer(t));
    ...
    <div className="flex flex-col gap-6" {...infobulle.handlers}>
      ...
      {hasFilters && (
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {selOrg && (
            <Chip label={accountLabel(useDataStore.getState().config, selOrg)} active onClear={() => setSelOrg(null)} />
          )}
          {selCat2 && (
            <Chip label={selCat2} active onClear={() => setSelCat2(null)} />
          )}
          {selType && (
            <Chip label={typeLabel(useDataStore.getState().config, selType)} active onClear={() => setSelType(null)} />
          )}
          <button
      ...
      {/* LineChart — Évolution mensuelle par compte */}
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          <div className="text-sm font-medium text-text">
            Évolution mensuelle des dépenses
            {selCat2 && <span className="text-text-sec"> &bull; {selCat2}</span>}
            {selType && <span className="text-text-sec"> &bull; {typeLabel(useDataStore.getState().config, selType)}</span>}
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] text-text-sec">
              Cliquez un point = mois &middot; Cliquez une légende = compte
            </span>
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height={chartHeight(300)}>
            ...
            >
              ...
              <Legend
                ...
                onClick={(e) => {
                  const org = String(e.dataKey ?? e.value);
                  if (organismes.some(o => o.id === org)) {
                    setSelOrg(selOrg === org ? null : org);
                  }
                }}
              />
              {organismes.map(({ id: org, label }) => (
                <Line
                  key={org}
                  type="monotone"
                  dataKey={org} name={label}
                  stroke={ORG_COLORS[org] ?? DONUT_COLORS[organismes.findIndex(o => o.id === org) % DONUT_COLORS.length]}
                  strokeWidth={selOrg && selOrg !== org ? 1 : 2.5}
                  strokeOpacity={selOrg && selOrg !== org ? 0.25 : 1}
                  dot={{ r: selOrg === org ? 4 : 2.5, strokeWidth: 0, fill: ORG_COLORS[org] }}
            ...
            <ResponsiveContainer width="100%" height={isSmall ? expByType.length * 42 + 24 : Math.max(280, expByType.length * 24 + 40)}>
              <BarChart data={expByType} layout="vertical" margin={isSmall ? { left: 0, right: 8 } : { left: 10, right: 20 }}>
                ...
                <Bar
                  ...
                  radius={[0, 4, 4, 0]}
                  onClick={(d) => {
                    if (d) setSelType(selType === d.id ? null : d.id);
                  }}
                  style={{ cursor: "pointer" }}
                >
                  {isSmall && (
                  ...
                  {expByType.map((e, i) => (
                    <Cell
                      key={i}
                      fill={selType === e.id ? "#818cf8" : TYPE_COLORS[i % TYPE_COLORS.length]}
                      fillOpacity={selType && selType !== e.id ? 0.25 : 1}
                    />
                  ))}
                </Bar>
              </BarChart>
