...
import { ENT_COLORS } from "@/config/colors";
...
export default function Salaire() {
  const { status, salary } = useDataStore();
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="card">
          <EmptyState
            ...
            title="Aucune donnée de salaire"
            description="Cette rubrique apparaîtra après l'import d'un classeur contenant des données de salaire."
          />
        </div>
      </div>
    );
      ...
      {entKPIs && (
        <div className="card">
          <div className="flex items-center gap-2 text-sm font-semibold text-text mb-3">
            {selEntreprise && (
              <span className="w-2.5 h-2.5 rounded-full" style={{ background: ENT_COLORS[selEntreprise] || "#6366F1" }} />
            )}
            {entKPIs.label}
        ...
        <ResponsiveContainer width="100%" height={chartHeight(320)}>
          <LineChart data={filteredHistData}>
            ...
            {!selEntreprise && entrepriseChanges.map((ch, i) => (
              <ReferenceLine key={i} x={ch.mk} stroke={ENT_COLORS[ch.entreprise] || "#6B7280"} strokeDasharray="4 3" strokeWidth={1.5}>
                ...
                <Label value={ch.entreprise} position="top" fill={ENT_COLORS[ch.entreprise] || "#6B7280"} fontSize={11} />
              </ReferenceLine>
            ))}
          </LineChart>
        </ResponsiveContainer>
