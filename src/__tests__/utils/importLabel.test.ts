...
describe("fmtImportOrigine", () => {
  it("nomme le fichier et la date de l'import", () => {
    expect(fmtImportOrigine("Budget 2025.xlsx", "2026-08-11T18:00:00.000Z"))
      .toBe("les donnees de « Budget 2025.xlsx », importees le 11/08/2026");
  });
  ...
  it("se passe de la date quand elle manque", () => {
    ...
    expect(fmtImportOrigine("Budget 2025.xlsx", null))
      .toBe("les donnees de « Budget 2025.xlsx »");
  });

  it("n'affiche jamais une date invalide", () => {
    expect(fmtImportOrigine("Budget 2025.xlsx", "pas-une-date"))
      .toBe("les donnees de « Budget 2025.xlsx »");
  });
});
