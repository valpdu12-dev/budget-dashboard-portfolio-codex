...
const MONTHS_MULTI: SalaryMonth[] = [
  // 2024 — Entreprise Initiale
  makeSalaryMonth({ mk: "2024-01", entreprise: "Entreprise Initiale", brut: 3200, net: 2500, cotSal: 600, indem: 50, retenues: 50 }),
  makeSalaryMonth({ mk: "2024-02", entreprise: "Entreprise Initiale", brut: 3200, net: 2500, cotSal: 600, indem: 50, retenues: 50 }),
  makeSalaryMonth({ mk: "2024-03", entreprise: "Entreprise Initiale", brut: 3200, net: 2500, cotSal: 600, indem: 50, retenues: 50 }),
  // 2024 — Entreprise Alpha (changement d'entreprise)
  makeSalaryMonth({ mk: "2024-04", entreprise: "Entreprise Alpha", brut: 3500, net: 2800, cotSal: 700, indem: 0, retenues: 0 }),
  makeSalaryMonth({ mk: "2024-05", entreprise: "Entreprise Alpha", brut: 3500, net: 2800, cotSal: 700, indem: 0, retenues: 0 }),
  makeSalaryMonth({ mk: "2024-06", entreprise: "Entreprise Alpha", brut: 3500, net: 2800, cotSal: 700, indem: 0, retenues: 0 }),
  // 2025 — Entreprise Alpha
  makeSalaryMonth({ mk: "2025-01", entreprise: "Entreprise Alpha", brut: 3600, net: 2900, cotSal: 700, indem: 0, retenues: 0 }),
  makeSalaryMonth({ mk: "2025-02", entreprise: "Entreprise Alpha", brut: 3600, net: 2900, cotSal: 700, indem: 0, retenues: 0 }),
  makeSalaryMonth({ mk: "2025-03", entreprise: "Entreprise Alpha", brut: 3700, net: 3000, cotSal: 700, indem: 0, retenues: 0 }),
];
...
describe("useSalaryPageData — histData & entrepriseChanges", () => {
  ...
  it("chaque point contient mk, label, Net, Brut, entreprise", () => {
    ...
    expect(first).toHaveProperty("entreprise", "Entreprise Initiale");
  ...
  it("détecte les changements d'entreprise", () => {
    seedSalary(MONTHS_MULTI);
    const { result } = renderHook(() => useSalaryPageData());
    // 2 entreprises : Entreprise Initiale (début) + Entreprise Alpha (à partir de 2024-04)
    expect(result.current.entrepriseChanges).toHaveLength(2);
    expect(result.current.entrepriseChanges[0]).toMatchObject({ mk: "2024-01", entreprise: "Entreprise Initiale" });
    expect(result.current.entrepriseChanges[1]).toMatchObject({ mk: "2024-04", entreprise: "Entreprise Alpha" });
  });
});
...
describe("useSalaryPageData — filteredHistData", () => {
  it("filtre par selEntreprise", () => {
    seedSalary(MONTHS_MULTI);
    useFilterStore.setState({ selEntreprise: "Entreprise Initiale" });
    const { result } = renderHook(() => useSalaryPageData());
    expect(result.current.filteredHistData).toHaveLength(3); // 3 mois Entreprise Initiale
    result.current.filteredHistData.forEach((d) => {
      expect(d.entreprise).toBe("Entreprise Initiale");
    });
  });
  ...
  it("combine selEntreprise + selYear", () => {
    seedSalary(MONTHS_MULTI);
    useFilterStore.setState({ selEntreprise: "Entreprise Alpha", selYear: "2024" });
    const { result } = renderHook(() => useSalaryPageData());
    // Entreprise Alpha en 2024 : avril, mai, juin
...
describe("useSalaryPageData — entreprises", () => {
  it("retourne la liste unique des entreprises", () => {
    seedSalary(MONTHS_MULTI);
    const { result } = renderHook(() => useSalaryPageData());
    expect(result.current.entreprises).toContain("Entreprise Initiale");
    expect(result.current.entreprises).toContain("Entreprise Alpha");
...
describe("useSalaryPageData — entKPIs", () => {
  ...
  it("filtre par entreprise quand selEntreprise est défini", () => {
    ...
    expect(kpis.label).toBe("Entreprise Initiale");
...
describe("useSalaryPageData — stackedData", () => {
  ...
  it("filtre par selEntreprise", () => {
    seedSalary(MONTHS_MULTI);
    useFilterStore.setState({ selEntreprise: "Entreprise Alpha" });
    const { result } = renderHook(() => useSalaryPageData());
    // Entreprise Alpha : 6 mois (2024-04 à 2024-06 + 2025-01 à 2025-03)
...
describe("useSalaryPageData — hasFilters", () => {
  ...
  it("clearRevFilters remet les filtres à null", () => {
    seedSalary(MONTHS_MULTI);
    useFilterStore.setState({ selEntreprise: "Entreprise Alpha", selYear: "2025" });
    const { result } = renderHook(() => useSalaryPageData());
