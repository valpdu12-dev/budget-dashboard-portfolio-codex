...
const BASE_TX: Transaction[] = [
  // Janvier 2025 — Alimentation Banque Horizon - Courant
  makeTx({ date: "2025-01-10", montant: 200, dc: "Débit", compte: "Banque Horizon - Courant", cat2: "Alimentation", cat3: "Marché local", type: "CB", ville: "Paris", monthKey: "2025-01" }),
  makeTx({ date: "2025-01-15", montant: 100, dc: "Débit", compte: "Banque Nova - Compte joint", cat2: "Alimentation", cat3: "Épicerie locale", type: "CB", ville: "Lyon", monthKey: "2025-01" }),
  // Janvier 2025 — Loisirs
  makeTx({ date: "2025-01-20", montant: 50, dc: "Débit", compte: "Banque Horizon - Courant", cat2: "Loisirs", cat3: "Cinéma", type: "CB", ville: "Paris", monthKey: "2025-01" }),
  // Janvier 2025 — Crédit (ne doit PAS être dans les dépenses)
  makeTx({ date: "2025-01-25", montant: 2500, dc: "Crédit", compte: "Banque Horizon - Courant", cat2: "", cat3: "Salaire", type: "Virement", ville: "", monthKey: "2025-01" }),
  // Février 2025
  makeTx({ date: "2025-02-10", montant: 300, dc: "Débit", compte: "Banque Horizon - Courant", cat2: "Logement", cat3: "Loyer", type: "Prélèvement", ville: "Paris", monthKey: "2025-02" }),
  makeTx({ date: "2025-02-15", montant: 80, dc: "Débit", compte: "Banque Équilibre - Compte joint", cat2: "Transport", cat3: "RATP", type: "CB", ville: "Paris", monthKey: "2025-02" }),
  // Mars 2025
  makeTx({ date: "2025-03-05", montant: 120, dc: "Débit", compte: "Banque Horizon - Courant", cat2: "Alimentation", cat3: "Marché local", type: "CB", ville: "Paris", monthKey: "2025-03" }),
  makeTx({ date: "2025-03-12", montant: 45, dc: "Débit", compte: "Carte repas - Titres restaurant", cat2: "Alimentation", cat3: "Boulangerie", type: "CB", ville: "Joinville", monthKey: "2025-03" }),
];

// Données 2024 pour la comparaison N vs N-1
const TX_2024: Transaction[] = [
  makeTx({ date: "2024-01-10", montant: 180, dc: "Débit", compte: "Banque Horizon - Courant", cat2: "Alimentation", cat3: "Marché local", type: "CB", ville: "Paris", monthKey: "2024-01" }),
  makeTx({ date: "2024-01-20", montant: 250, dc: "Débit", compte: "Banque Horizon - Courant", cat2: "Logement", cat3: "Loyer", type: "Prélèvement", ville: "Paris", monthKey: "2024-01" }),
];
...
describe("useExpenseData — expMonthlyLines", () => {
  ...
  it("ventile les montants par organisme (Banque Horizon, Banque Nova, etc.)", () => {
    seedStores(BASE_TX, { period: "all" });
    const { result } = renderHook(() => useExpenseData());
    const jan = result.current.expMonthlyLines.find((r) => r.mk === "2025-01");
    expect(jan).toBeDefined();
    if (jan) {
      // Banque Horizon - Courant : 200 (Alimentation) + 50 (Loisirs) = 250 → Banque Horizon
      expect(jan["Banque Horizon"]).toBe(250);
      // Banque Nova - Compte joint : 100 → Banque Nova
      expect(jan["Banque Nova"]).toBe(100);
  ...
  it("filtre par selType quand défini", () => {
    ...
    if (feb) {
      ...
      expect(feb["Banque Horizon"]).toBe(300);
    }
  });
});
...
describe("useExpenseData — topMerchants", () => {
  ...
  it("exclut les cat3 vides ou 'x'", () => {
    const tx = [
      ...
      makeTx({ date: "2025-01-03", montant: 300, dc: "Débit", cat3: "Marché local", monthKey: "2025-01" }),
    ];
    seedStores(tx, { period: "all" });
    const { result } = renderHook(() => useExpenseData());
...
describe("useExpenseData — compNvsN1", () => {
  ...
  it("ignore une catégorie de N-1 située HORS de la fenêtre comparée", () => {
    ...
    const tx = [
      ...
      makeTx({ date: "2025-01-01", montant: 50, dc: "Débit", cat2: "Alimentation", cat3: "Marché local", type: "CB", monthKey: "2025-01" }),
    ];
    seedStores(tx, { period: "all" });
    const { result } = renderHook(() => useExpenseData());
  ...
  it("exclut les transferts de la comparaison 2024", () => {
    const txWithTransfer = [
      ...BASE_TX,
      makeTx({ date: "2024-01-10", montant: 500, dc: "Débit", cat2: "Alimentation", type: "Transfert Horizon vers Équilibre", monthKey: "2024-01" }),
    ];
