  ...
  it("exclut les transferts", () => {
    const withTransfer = [
      ...
      makeTx({ date: "2025-03-09", cat2: "Trans", montant: 999, dc: "Débit",
               type: "Transfert Horizon vers Équilibre", monthKey: "2025-03" }),
    ];
...
describe("useInsights — récurrents", () => {
  it("détecte un paiement récurrent sur 3+ mois", () => {
    const txs = [
      makeTx({ date: "2025-01-01", cat3: "Netflix", montant: 13.99, dc: "Débit", monthKey: "2025-01" }),
      makeTx({ date: "2025-02-01", cat3: "Netflix", montant: 13.99, dc: "Débit", monthKey: "2025-02" }),
      makeTx({ date: "2025-03-01", cat3: "Netflix", montant: 13.99, dc: "Débit", monthKey: "2025-03" }),
    ];
    const { result } = renderHook(() => useInsights(txs, "2025-03", "2025-02"));
    const netflix = result.current.recurring.find((r) => r.name === "Netflix");
    expect(netflix).toBeDefined();
    expect(netflix!.montant).toBe(13.99);
    expect(netflix!.freq).toBe("3 mois");
  });
