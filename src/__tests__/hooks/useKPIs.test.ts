...
describe("useKPIs — dépenses et recettes", () => {
  const txs = [
    ...
    makeTx({ date: "2025-03-05", montant: 500,  dc: "Débit",  monthKey: "2025-03", type: "Transfert Horizon vers Équilibre" }),
  ];
...
describe("useKPIs — soldes", () => {
  it("retourne les soldes du mois courant et précédent", () => {
    const balances = {
      "2025-02": { "Banque Horizon - Courant": 4000, "Banque Équilibre - Compte joint": 1500 },
      "2025-03": { "Banque Horizon - Courant": 4500, "Banque Équilibre - Compte joint": 1600 },
    };
    const { result } = renderHook(() =>
      useKPIs([], balances, [], "2025-03", "2025-02")
    );
    expect(result.current.curBal["Banque Horizon - Courant"]).toBe(4500);
    expect(result.current.prevBal["Banque Horizon - Courant"]).toBe(4000);
  });
