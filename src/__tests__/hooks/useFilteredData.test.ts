...
describe("useFilteredData — filtres cat1 et transferts", () => {
  ...
  it("exclut les transferts par défaut", () => {
    ...
    expect(types).not.toContain("Transfert Horizon vers Équilibre");
  });
  ...
  it("inclut les transferts quand showTransfers=true", () => {
    ...
    const { result } = renderHook(() => useFilteredData());
    const types = result.current.baseTx.map((t) => t.type);
    expect(types).toContain("Transfert Horizon vers Équilibre");
  });
...
describe("useFilteredData — drill-down filteredTx", () => {
  beforeEach(() => {
    const txs = [
      makeTx({ date: "2025-03-01", cat2: "Alimentation", type: "CB", compte: "Banque Horizon - Courant", monthKey: "2025-03" }),
      makeTx({ date: "2025-03-02", cat2: "Transport",    type: "CB", compte: "Banque Équilibre - Compte joint", monthKey: "2025-03" }),
    ];
    useDataStore.getState().setData(txs, makeSalaryData(), makeConfig());
    useFilterStore.getState().setPeriod("1M");
  });
  ...
  it("filtre par selOrg (organisme)", () => {
    useFilterStore.getState().setSelOrg("Banque Équilibre");
    const { result } = renderHook(() => useFilteredData());
    expect(result.current.filteredTx).toHaveLength(1);
    expect(result.current.filteredTx[0].compte).toBe("Banque Équilibre - Compte joint");
  });
});
