...
function makeSavingsTxs() {
  return [
    // Entrées épargne (type Épargne Horizon = EPARGNE_TYPES)
    makeTx({ date: "2025-01-05", type: "Épargne Horizon", dc: "Débit", montant: 300, monthKey: "2025-01", compte: "Banque Horizon - Courant" }),
    makeTx({ date: "2025-02-05", type: "Épargne Horizon", dc: "Débit", montant: 300, monthKey: "2025-02", compte: "Banque Horizon - Courant" }),
    makeTx({ date: "2025-03-05", type: "Épargne Horizon", dc: "Débit", montant: 300, monthKey: "2025-03", compte: "Banque Horizon - Courant" }),
    // Sortie épargne
    makeTx({ date: "2025-03-15", type: "Retrait épargne", dc: "Débit", montant: 100, monthKey: "2025-03", compte: "Retrait épargne" }),
    // Recettes
    makeTx({ date: "2025-03-20", type: "Virement", dc: "Crédit", montant: 2500, monthKey: "2025-03", compte: "Banque Horizon - Courant" }),
  ];
}
...
describe("useSavingsData — KPIs", () => {
  ...
  it("retourne ratio=0 quand recettes=0", () => {
    const txs = [makeTx({ date: "2025-03-05", type: "Épargne Horizon", dc: "Débit", montant: 200, monthKey: "2025-03", compte: "Banque Horizon - Courant" })];
...
describe("useSavingsData — donutData", () => {
  it("agrège par type", () => {
    const txs = [
      makeTx({ date: "2025-03-01", type: "Épargne Horizon",       dc: "Débit", montant: 200, monthKey: "2025-03", compte: "Banque Horizon - Courant" }),
      makeTx({ date: "2025-03-02", type: "Épargne Nova", dc: "Débit", montant: 150, monthKey: "2025-03", compte: "Banque Horizon - Courant" }),
    ];
    useDataStore.getState().setData(txs, makeSalaryData(), makeConfig());
    const { result } = renderHook(() => useSavingsData());
    expect(result.current.donutData).toHaveLength(2);
    const ca = result.current.donutData.find((d) => d.name === "Épargne Horizon");
    expect(ca?.value).toBe(200);
  });

  it("filtre par selEpMonth si défini", () => {
    const txs = [
      makeTx({ date: "2025-02-01", type: "Épargne Horizon", dc: "Débit", montant: 100, monthKey: "2025-02", compte: "Banque Horizon - Courant" }),
      makeTx({ date: "2025-03-01", type: "Épargne Horizon", dc: "Débit", montant: 200, monthKey: "2025-03", compte: "Banque Horizon - Courant" }),
    ];
    useDataStore.getState().setData(txs, makeSalaryData(), makeConfig());
