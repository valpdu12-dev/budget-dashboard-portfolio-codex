...
function year(y: string, net: number, entreprise = "Entreprise Alpha"): SalaryMonth[] {
...
function fullSalary() {
  const months: SalaryMonth[] = [
    ...year("2013", 1000, "Entreprise Gamma"),
    ...year("2014", 1100),
    ...year("2024", 2000),
    ...year("2025", 2100),
...
describe("useSalaryInflationData — agrégation & filtre employeur", () => {
  ...
  it("filtre par employeur (Entreprise Alpha exclut 2013/Entreprise Gamma)", () => {
    const { result } = renderHook(() => useSalaryInflationData());
    expect(result.current.employers.map((e) => e.name)).toEqual(
      expect.arrayContaining([EMPLOYER_ALL, "Entreprise Gamma", "Entreprise Alpha"])
    );
    expect(result.current.yearly.some((y) => y.year === "2013")).toBe(true);
    act(() => result.current.setEmployer("Entreprise Alpha"));
