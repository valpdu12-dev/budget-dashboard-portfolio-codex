...
describe("useExcelWorker — parse()", () => {
  ...
  it("passe en success et stocke pendingData + validation sur message 'result'", () => {
    ...
    expect(result.current.pendingData?.transactions).toHaveLength(6);
...
describe("useExcelWorker — apply()", () => {
  it("bloque l'application d'un rapport invalide et conserve le jeu courant", () => {
    useDataStore.getState().setImportedDataset(MOCK_DATASET, "ancien.xlsx", "2026-09-15T00:00:00Z");
    const { result } = renderHook(() => useExcelWorker());
    act(() => result.current.parse(new ArrayBuffer(4)));
    act(() => workerInstance?.simulateMessage({ type: "result", dataset: MOCK_DATASET, validation: { ...MOCK_VALIDATION, ok: false } }));
    expect(result.current.status).toBe("error"); expect(result.current.pendingData).toBeNull();
    act(() => expect(result.current.apply("nouveau.xlsx")).toBe(false));
    expect(useDataStore.getState().importFileName).toBe("ancien.xlsx");
  });
  it("signale un stockage saturé tout en affichant les données valides", () => {
    const { result } = renderHook(() => useExcelWorker());
    act(() => result.current.parse(new ArrayBuffer(4)));
    act(() => workerInstance?.simulateMessage({ type: "result", dataset: MOCK_DATASET, validation: MOCK_VALIDATION }));
    const spy = vi.spyOn(Storage.prototype, "setItem").mockImplementation(() => { throw new Error("quota"); });
    act(() => expect(result.current.apply("test.xlsx")).toBe(true));
    expect(useDataStore.getState().storageNotice).toContain("ne seront pas conservées");
    expect(useDataStore.getState().config).toEqual(migrateDataset(MOCK_DATASET).config);
    spy.mockRestore();
  });
  it("appelle setUploadData dans le store quand pendingData est défini", () => {
    ...
    expect(store.isFromUpload).toBe(true);
    expect(store.transactions).toHaveLength(6);
