...
const ROWS: Row[] = [
  { date: "2026-01-15", label: "Épicerie locale Joinville", compte: "CA", montant: -42 },
...
describe("DataTable — colonnes prioritaires en mode carte", () => {
  it("affiche les colonnes prioritaires et masque les autres du DOM", () => {
    ...
    expect(screen.getAllByText("Épicerie locale Joinville")).toHaveLength(1);
  ...
  it("retombe sur les 3 premières colonnes quand aucune priority n'est déclarée", () => {
    ...
    expect(screen.getByText("Épicerie locale Joinville")).toBeInTheDocument();
    expect(screen.getByText("CA")).toBeInTheDocument();
...
describe("DataTable — fonctions préservées en mode carte", () => {
  ...
  it("trie réellement les cartes, et inverse le sens au clic sur la flèche", () => {
    ...
    const libelles = () =>
      [...container.querySelectorAll("[data-testid], div")].length && screen.getAllByText(/Épicerie locale|Boulangerie|Abonnement/).map((e) => e.textContent);
    ...
    expect(libelles()?.[0]).toBe("Épicerie locale Joinville");
  });
